...
import { a as getNewReleaseFilms, f as getTopFilms } from "./apiClient-BBCAVRqq.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/genres-DuBGpt0y.js
...
var GENRES = [
	{
		...
		matches: (f) => {
			...
			return s.includes("action") || s.includes("war") || s.includes("race") || s.includes("fight") || s.includes("battle") || s.includes("mission") || s.includes("soldier") || s.includes("combat") || s.includes("explosion") || s.includes("martial") || s.includes("chase") || t.includes("f1") || t.includes("superman") || t.includes("warfare") || t.includes("mission impossible") || t.includes("captain america") || t.includes("thunderbolts") || t.includes("accountant");
		}
	},
	{
		id: "scifi",
		...
		matches: (f) => {
			const s = (f.synopsis || "").toLowerCase();
			const t = (f.title || "").toLowerCase();
			return s.includes("sci-fi") || s.includes("science fiction") || s.includes("alien") || s.includes("space") || s.includes("future") || s.includes("robot") || s.includes("clone") || s.includes("planet") || s.includes("dystopian") || s.includes("cyberpunk") || s.includes("dimension") || t.includes("dune") || t.includes("avatar") || t.includes("mickey 17") || t.includes("elio") || t.includes("klara") || t.includes("man from nowhere");
		}
	},
	{
		id: "thriller",
		...
		matches: (f) => {
			const s = (f.synopsis || "").toLowerCase();
			const t = (f.title || "").toLowerCase();
			return s.includes("thriller") || s.includes("mystery") || s.includes("murder") || s.includes("conspiracy") || s.includes("crime") || s.includes("detective") || s.includes("framed") || s.includes("spy") || s.includes("serial") || s.includes("heist") || s.includes("noir") || s.includes("investigation") || t.includes("black bag") || t.includes("novocaine") || t.includes("alto knights");
		}
	},
	{
		id: "comedy",
		...
		matches: (f) => {
			const s = (f.synopsis || "").toLowerCase();
			const t = (f.title || "").toLowerCase();
			return s.includes("comedy") || s.includes("hilarious") || s.includes("laugh") || s.includes("funny") || s.includes("humorous") || s.includes("slapstick") || s.includes("satire") || s.includes("parody") || t.includes("minecraft") || t.includes("bugonia") || t.includes("honey don't");
		}
	},
	{
		id: "drama",
		...
		matches: (f) => {
			const s = (f.synopsis || "").toLowerCase();
			const t = (f.title || "").toLowerCase();
			return s.includes("drama") || s.includes("family") || s.includes("relationship") || s.includes("biography") || s.includes("historical") || s.includes("social") || s.includes("struggles") || s.includes("intimate") || s.includes("poignant") || t.includes("anora") || t.includes("wicked") || t.includes("conclave") || t.includes("brutalist") || t.includes("complete unknown") || t.includes("snow white") || t.includes("the bride");
		}
	},
	{
		id: "romance",
		...
		matches: (f) => {
			const s = (f.synopsis || "").toLowerCase();
			return s.includes("love") || s.includes("romance") || s.includes("romantic") || s.includes("wedding") || s.includes("heart") || s.includes("couple") || s.includes("affair") || s.includes("marry") || s.includes("dating") || s.includes("passion") || s.includes("soulmate");
		}
	},
	{
		id: "animation",
		...
		matches: (f) => {
			const s = (f.synopsis || "").toLowerCase();
			const t = (f.title || "").toLowerCase();
			return s.includes("animat") || s.includes("anime") || s.includes("cartoon") || s.includes("animated") || s.includes("cgI") || t.includes("elio") || t.includes("lilo") || t.includes("minecraft") || t.includes("dragon") || t.includes("inside out") || t.includes("how to train") || t.includes("zootopia") || t.includes("ne zha") || t.includes("wild robot");
		}
	},
	{
		id: "horror",
		...
		matches: (f) => {
			const s = (f.synopsis || "").toLowerCase();
			const t = (f.title || "").toLowerCase();
			return s.includes("horror") || s.includes("vampire") || s.includes("ghost") || s.includes("demon") || s.includes("monster") || s.includes("dead") || s.includes("zombie") || s.includes("supernatural") || s.includes("cult") || s.includes("haunted") || s.includes("slasher") || t.includes("nosferatu") || t.includes("sinners") || t.includes("heretic") || t.includes("28 years") || t.includes("until dawn") || t.includes("death of a unicorn") || t.includes("final destination") || t.includes("substance");
		}
	}
];
function GenresPage() {
