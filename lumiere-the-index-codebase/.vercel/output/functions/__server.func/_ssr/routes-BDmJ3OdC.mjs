...
import { _ as tmdbBackdropUrl, a as getNewReleaseFilms, c as getTmdbMovieVideos, f as getTopFilms, h as searchTmdbMovie, i as getNewEntries, o as getRisingFilms, p as getTrendingFilms, r as getLiveStats, v as tmdbPosterUrl } from "./apiClient-BBCAVRqq.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/routes-BDmJ3OdC.js
...
var GENRE_CATEGORIES = [
	{
		...
		matches: (f) => {
			...
			return s.includes("action") || s.includes("fight") || s.includes("race") || s.includes("war") || s.includes("battle") || s.includes("combat") || s.includes("chase") || s.includes("explosion") || s.includes("martial") || s.includes("mission") || s.includes("soldier") || t.includes("f1") || t.includes("superman") || t.includes("warfare") || t.includes("captain america") || t.includes("thunderbolts");
		}
	},
	{
		id: "scifi",
		...
		matches: (f) => {
			const s = (f.synopsis || "").toLowerCase();
			const t = (f.title || "").toLowerCase();
			return s.includes("sci-fi") || s.includes("science fiction") || s.includes("future") || s.includes("alien") || s.includes("space") || s.includes("robot") || s.includes("clone") || s.includes("planet") || s.includes("dystopian") || s.includes("dimension") || t.includes("dune") || t.includes("avatar") || t.includes("mickey 17") || t.includes("elio") || t.includes("man from nowhere");
		}
	},
	{
		id: "indie",
		...
		matches: (f) => {
			const s = (f.synopsis || "").toLowerCase();
			const t = (f.title || "").toLowerCase();
			return s.includes("drama") || s.includes("award") || s.includes("festival") || s.includes("sundance") || s.includes("cannes") || s.includes("venice") || s.includes("berlin") || s.includes("independent") || s.includes("indie") || s.includes("arthouse") || s.includes("auteur") || s.includes("character study") || t.includes("substance") || t.includes("anora") || t.includes("brutalist") || t.includes("complete unknown") || t.includes("emilia perez") || t.includes("nosferatu") || t.includes("i'm still here");
		}
	},
	{
		id: "asian",
	...
	},
	{
		...
		matches: (f) => {
			const s = (f.synopsis || "").toLowerCase();
			const t = (f.title || "").toLowerCase();
			return s.includes("animat") || s.includes("anime") || s.includes("cartoon") || s.includes("animated") || t.includes("elio") || t.includes("lilo") || t.includes("minecraft") || t.includes("dragon") || t.includes("inside out") || t.includes("how to train") || t.includes("ne zha") || t.includes("wild robot") || t.includes("zootopia");
		}
	},
	{
		id: "thriller",
		...
		matches: (f) => {
			const s = (f.synopsis || "").toLowerCase();
			const t = (f.title || "").toLowerCase();
			return s.includes("thriller") || s.includes("mystery") || s.includes("murder") || s.includes("conspiracy") || s.includes("crime") || s.includes("detective") || s.includes("spy") || s.includes("heist") || s.includes("noir") || s.includes("investigation") || s.includes("horror") || t.includes("black bag") || t.includes("novocaine") || t.includes("alto knights") || t.includes("sinners");
		}
	}
];
var ROW_LIMIT = 10;
var MIN_FILMS_FOR_FULL_ROW = 5;
/**
...
function GenreSections() {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
		className: "mt-8 space-y-10 px-4 lg:px-6",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "text-[10px] uppercase tracking-[0.22em] text-muted-foreground",
